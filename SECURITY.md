...
## 1. S3 publish helper - `scripts/publish-s3.sh`
...
- **Preferred shipped state is offline.** The documented best practice (`references/diagram-types.md`) is to render diagrams to **inline SVG** for the shipped artifact, dropping both the CDN and the JS dependency. The live CDN path is for drafting and iteration only.
